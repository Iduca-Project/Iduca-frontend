...
// Função para buscar TODOS os dados da página (curso e prova)
async function getPageData(courseId: string) {
    ...
    // A chamada da prova pode falhar (ex: se não houver prova), então tratamos isso com calma
    const examData = examRes.ok ? await examRes.json() : null;

    return { courseData, examData };
}

const SelectCoursePage = async ({ params }: PageProps) => {
    const { courseId } = params;

    const { courseData, examData } = await getPageData(courseId);

    if (!courseData) return <div>Curso não encontrado.</div>;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-10 p-1">
                    <div className="flex flex-col gap-5 justify-around">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {/* O link agora aponta para a primeira aula do primeiro módulo */}
                            <Link href={getLinkForContent(courseData.modules[0]?.content[0])} className="w-full md:w-3/6 self-start">
